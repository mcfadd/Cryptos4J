...
/**
 * Historical data class that is equivalent to the json object returned by the <i>min-api.cryptocompare.com/data/histominute</i> end point.
 * 
 * @author Matt - github.com/mcfadd
 * @since Cryptos4J v1.0
 * @see TimestampedData
 */
public class HistoricalMinute extends HistoricalData {

	protected HistoricalMinute(String currencyTo, String currencyFrom, CryptoExchanges.exchanges exchange) {
	...
	public TimestampedData getDataAtTime(long timestamp) throws TimestampOutOfBoundsException {
		...
		try {
			...
			long offset;
			if ((offset = (timestamp % 60)) != 0) {
				offset = 60 - offset;
			}

			return Data[(Data.length - 1
					- (int) (this.getTimeTo() - (timestamp + offset)) / (60 * this.getAggregate()))];
	...
	protected void updateURL() {
		try {
			...
			URL temp = new URL("https://min-api.cryptocompare.com/data/histominute?fsym=" + this.getCurrencyFrom()
